...
22. PanePilot-owned tmux sessions carry versioned session-scoped `@panepilot_*` metadata. Live tagged sessions are discovered with one-shot tmux commands and reconciled into the local database by stable terminal UUID and canonical project folder. This applies both over SSH and on the current host, because a session created by another PanePilot client over SSH must also appear in PanePilot running locally on that host. Tmux is authoritative for live-session presence; SQLite remains authoritative for durable local workspace state.
...
27. Every folder-backed project has multiple Markdown notes stored under `.panepilot/notes/*.md`. The files are authoritative on the local or SSH host and are never mirrored into SQLite. Notes default to the same sanitized GitHub-style Preview used by Files, retain a per-note Monaco Source mode, and render the current unsaved draft. A legacy `.notes-panepilot` file migrates into the notes folder.
...
35. Google Drive upload destinations are project-scoped and use locally configured rclone remotes. Each project selects one named remote (allowing a different Google account) and one existing folder path. rclone owns OAuth credentials; PanePilot stores no Google tokens and never automatically creates a public `rclone link`. After an upload, the user may explicitly create, copy, and revoke an anyone-with-the-link URL from the Files workspace. Removing a PanePilot destination or project never deletes uploaded Drive files.
36. Project keyboard navigation is renderer-local and applies only to the focused workspace pane. Command + `/` directly toggles discoverable KeyTips on macOS. Holding Control and tapping `?` three times within the gesture window remains the terminal-safe alternative: the first tap passes through to the focused editor; for a focused xterm, PanePilot explicitly writes the standard terminal control byte because xterm does not natively translate Control + Shift + `/`. The second and third taps are consumed so a completed gesture reaches the terminal only once. The exact targeted terminal session comes from the focused ManagedTerminal DOM owner; shortcut routing must never write to a background session. Single letters route to project capabilities, Command/Control + `1`–`9` selects active terminal/chat tabs, and Command/Control + Shift + `[` or `]` cycles those tabs.
37. Sidebar project sorting is client-local and independently persisted per connection; sorting never moves a project outside its local/SSH machine group. A project glyph uses distinct left and right halves to show pane membership. Command/Control + Shift + `\` swaps the left and right pane contents while preserving focus on the same project; KeyTips exposes the same operation as `S`.
38. A project may persist one Unicode grapheme as its custom sidebar icon. The glyph itself opens the picker, while its violet left half and teal right half remain pane-membership signals. The focused project path is the only copy target in the top bar and uses a dotted underline instead of a separate copy icon.
39. Escape dismisses only the topmost active modal. A modal must register its own state-aware dismiss callback; critical save, launch, or connection work consumes Escape without closing until the operation settles. Fixed modal backdrops must not be trapped inside per-pane stacking contexts.
40. Renderer requests that open a terminal/LaTeX launcher or reveal a selected session are one-shot events targeted to one project and one split pane. The receiving workspace acknowledges the request immediately, and ordinary project switching clears any locally open launcher so canceled dialogs cannot replay or migrate to another project.
41. Project “Newest first” and recency-ordered sidebar terminal entries use client-local selection history: the item most recently selected by the user comes first. Main-workspace terminal tabs are exempt because their order is manual. Items without recorded selection history use their existing timestamp fallback, and “Newest added” remains the explicit project-creation ordering.
...
47. A Git repository with no `origin` remote shows Local in the top Repository action, and that action opens the read-only Git pane. A configured GitHub origin shows Public, Private, or Internal instead. PanePilot asks the locally authenticated GitHub CLI first so private repositories can be identified, falls back to GitHub's public repository API, caches the read briefly, and shows Unknown rather than guessing when neither source can see the repository. This indicator never changes repository visibility or remotes.
48. Application readability is client-local and controls Electron's renderer zoom as one bounded scale, so interface text, xterm, Monaco, and rendered documents grow together without separate drifting preferences. Comfortable 110% is the first-run default, and the toolbar exposes 90%, 100%, 110%, and 125% choices. Interface zoom has no keyboard shortcut: Command/Control + plus, minus, and zero are suppressed, while workspace history uses the primary-modifier arrow gestures.
49. Quick Codex chats are project-scoped, tmux-backed `temporary-chat` sessions. They are opened from Command-K or the top toolbar, never appear as ordinary terminal tabs/sidebar sessions, survive project switching and app restarts, and remain until the user explicitly clears them. Clearing stops the exact backend session and removes PanePilot's saved output but never deletes Codex's provider archive.
50. An ordinary, non-archived terminal session may be transferred only to another active project on the same connection. A live tmux session must be retagged with the destination project identity/path before SQLite ownership changes; capability-owned Action, project Q&A, quick-chat, and LaTeX-chat sessions are never transferable. A running process keeps its current working directory until force reload/resume, while later launches use the destination folder.
51. The Files capability may create empty files/folders and rename existing entries on local or SSH projects. Mutations are bounded to the canonical project root, accept one validated basename rather than an arbitrary destination path, never overwrite an existing entry, and reject final-component symlinks for rename. Monaco and the explorer expose copy-path, rename, and file-download actions; open drafts are path-remapped after a rename without silently saving their unsaved content.
52. The selected conversation in LLM Chats exposes a visible copy action for its exact provider session ID. The shortened identifier remains supporting metadata only; successful copying gives brief inline feedback and never changes the provider archive.
53. New Project supports either an existing folder or one newly created beneath a chosen local/SSH parent. The new folder accepts one validated name, never overwrites an existing entry, becomes the default editable project name, and is created before the project record. A new LaTeX folder also receives its normalized main file with a minimal document so the existing LaTeX validation remains true. The SSH folder picker is one continuous browser containing its editable canonical path, inline new-folder action, and folder/file listing. Typing after the current path fuzzy-filters and ranks the visible entries without another remote request; only folders navigate. The best folder match is selection one after each query or navigation, arrows change selection, Tab completes it, and slash or Enter enters it. Slash-separated input therefore navigates one verified remote directory at a time.
54. A LaTeX writing chat persists its project/section scope and Ask/Edit mode before its tmux session launches, so the first remote metadata snapshot is portable to another PanePilot client. A stale ordinary-terminal snapshot must never downgrade a richer local LaTeX attachment; the client repairs that tmux metadata when it can identify the existing chat.
55. The LaTeX PDF preview is a continuous, lazily rendered page stack. Once opened, it remains mounted while the user visits other project capabilities, and its tab, page, zoom, and scroll position survive project switching during the current renderer lifetime. Recompiling preserves the reading position. Recompile runs bounded `latexmk -pdf -cd` on the project machine and then reloads the resulting PDF.
56. A successfully loaded LaTeX workspace map is cached per project for the current renderer lifetime. Only the first uncached visit uses the manuscript-mapping screen; later visits render the cached sections immediately without automatically rescanning the project. Explicit Rescan and structural source saves refresh the map in the background. Late SSH responses must never replace the newly selected project, and a background refresh failure must leave the cached editor usable.
57. Manuscript and PDF Preview share one persistent right-side writing-chat margin. The margin may be hidden or resized by pointer or keyboard without creating a second chat instance; visibility and bounded width are client-local preferences stored independently per LaTeX project.
58. LaTeX Auto compile is a project-scoped, client-local preference and is off by default. When enabled, the active client polls a bounded metadata-only revision of visible `.tex` files and runs `latexmk` only after that revision changes. After its initial snapshot, the PDF preview may replace its document only through manual Recompile or an enabled auto-compile run; it never silently adopts an externally replaced PDF from disk.
59. The LaTeX writing-chat margin keeps only two persistent control rows: a compact pane heading and one active-chat identity/mode row. A single chat is not rendered as a redundant switcher; multiple chats use an on-demand menu. Reload, archived chats, and pane hiding live in one overflow menu, while the pane owns the sole visible new-chat action. The project toolbar exposes Chat only when the margin is hidden.
60. The LaTeX Document Map is independently hideable through a client-local, per-project layout preference. Source comments are durable project-scoped annotations stored in the authoritative `.panepilot/latex-comments.json` file beside other shared project metadata; the file omits PanePilot's client-local project ID so another local or SSH client can read the same comments. Existing SQLite comments migrate into that file only when it does not exist. Each comment attaches to an exact saved `.tex` selection plus bounded surrounding anchors. Monaco highlights resolvable selections and renders visible-file comments in a proofing margin aligned to editor line positions; the cards follow editor scrolling, and a genuinely missing or ambiguous selection is marked as moved rather than silently reattached. Comment creation saves an unsaved draft first, validates the exact selection in the main process, and never invokes an agent.
61. Hovering an image path inside a single-line LaTeX `\includegraphics{...}` command resolves bounded project-relative image candidates and shows the file through the existing preview IPC. The hover image and its explicit link open that exact path in Files. Text previews and edits remain capped at 1 MB, while supported image previews may transfer up to 5 MB locally or over SSH.
62. LaTeX source auto-save is a client-local, per-project preference and is on by default. After a short typing pause it saves the exact draft that triggered the run; later typing remains dirty and schedules another save. Ordinary saves do not rescan the document map, while changes to section/include commands request a background map refresh. A failed automatic save is shown non-destructively and is not retried in a loop until the draft changes or the user saves manually.
...
64. Selecting saved or unsaved LaTeX source in the Manuscript Monaco editor exposes one anchored inline-edit prompt. PanePilot saves the draft before dispatch, verifies the exact saved file text and Monaco line/column range in the main process, and includes the selected paragraph plus one neighboring paragraph on each side as context. Each LaTeX project has at most one hidden, project-scoped, tmux-backed Codex inline-edit runner identity; it is excluded from the sidebar and Writing chats pane. Every ask launches a fresh one-shot `codex exec` turn in that exact project rather than driving the interactive TUI or sending `/clear`. Inline completion uses asynchronous, low-frequency tmux checks and does not continuously rescan the manuscript. Monaco stays writable, tracks the requested range while Codex works, and merges the accepted replacement into the live draft before supplemental workspace/history refreshes finish, without discarding unrelated typing or resetting the current view. Saving LaTeX source in that same project remains disabled only until the pending revision settles so disk validation cannot overwrite concurrent work. PanePilot accepts the result only when Codex changed the exact selected range and no other LaTeX source. The Save-adjacent Edits control exposes an accumulated editorial trail containing the instruction, original selection, replacement, final model note, and status. A completed revision may be safely rolled back only while its replacement can still be located exactly; deleting an entry removes history only. Edit snapshots and highlights remain independent and persist until the user explicitly clears them.
65. During the current renderer lifetime, every LaTeX project remembers its last open manuscript source, exact Monaco cursor/selection, horizontal scroll, and vertical scroll. Manuscript and an opened PDF remain mounted while switching between LaTeX capabilities, so tab changes do not reconstruct Monaco or PDF.js. A bounded source/model cache restores saved and unsaved source immediately after a project workspace remount. PDF Preview independently remembers the project’s page, continuous-scroll offset, and zoom and flushes the latest reading position when its viewer unmounts.
66. Renderer-requested local LaTeX source scans, local PDF reads, SSH source scans, SSH context checks, and SSH comment reads/writes use asynchronous filesystem or child-process paths. Inline-edit verification still checks every bounded `.tex` source before accepting a result, but that work must yield Electron's main event loop so other projects, tabs, scrolling, and typing remain responsive.
67. Project and terminal switching must use targeted renderer updates rather than reloading the complete project collection. Renderer project hydration omits saved terminal output; the renderer requests the exact session replay only when its terminal surface first attaches. Each physical pane retains at most four recently used ordinary xterm surfaces across terminal and LaTeX project switches, with hidden surfaces barred from input and resize IPC and least-recently-used surfaces disposed without stopping their backend session. Terminal lifecycle events patch local state directly, metadata events coalesce one project-scoped refresh, and tmux discovery runs once per visible connection without a zero-change refresh. Focused-project Git reads keep their last bounded cached result while refreshing in the background. Development builds log project-selection paint time, terminal readiness/focus, project-list payload size, and discovery duration.
68. Workspace navigation history is client-local and persists the last twenty unique destinations in most-recent access order across projects, ordinary terminal tabs, and type-specific capabilities. Command/Control + Up or Down opens a translucent switcher bounded to the left sidebar. Its differently marked current destination is pinned first, followed by at most seven valid prior destinations; the first press selects the most recent prior destination, repeated Up/Down presses move the sliding highlight through every rendered row including Current, and a confirmed primary-modifier release opens the selected destination in the focused pane. Release confirmation requires a trusted, non-repeated key-up with that modifier no longer held, followed by 120 ms without renewed held-key evidence; there is no timeout while the key is held. Closing, window blur, hidden-document transitions, and unmount cancel pending navigation. Only while that switcher is already open, primary-modifier Right cycles Recent history → Project tools → Project terminals → Recent history; Left reverses this cycle. All three views share a large title above a vertically centered, evenly spaced pill list; Tools and terminals show the highlighted project's icon and name, retaining that project through an empty terminal list. Tools show only non-terminal capability names and initially select Actions (Manuscript for LaTeX); project terminals use most-recent access order, with client-local session selection recency and then creation time as fallbacks, without changing manual workspace tab order. When the switcher is closed, primary-modifier Left/Right pass untouched to the focused editor or terminal. All three views retain keyboard selection and scroll when necessary. On view changes, the selected project's icon and name travel into that heading as unrelated rows fade away. Exact matching terminal rows move independently to their new sorted positions, including multiple history entries belonging to the same project; newly revealed options fan out from the project. Rapid keyboard navigation cancels superseded motion and reduced-motion preferences skip it. Pointer hover and clicks never move the selection or open a destination. Pressing `D` while the primary modifier remains held removes the hovered recent destination, falling back to the keyboard-selected row when there is no hover target. The row swipes left before the next older history item fills its place, with immediate removal for reduced-motion preferences. Removal persists immediately; releasing the modifier during the animation closes the switcher without opening the deleted destination. Current and the project drill-down lists are not removable. Electron application-menu accelerators are ignored only while the switcher is open and restored on close, blur, or navigation. Recent and terminal rows lead with the project's current custom or fallback glyph and overlay a color-coded capability sub-icon. Only an ordinary terminal destination may show its exact terminal's Working, Needs attention, or Flagged indicator; Files, Notes, Actions, other capabilities, and LaTeX destinations never borrow aggregate project status. Archived or deleted projects and missing sessions are skipped. Escape or window blur cancels without navigating. Selecting a terminal expands its project when necessary and minimally scrolls that exact row into the visible area of the sidebar; manually collapsing the currently selected project remains allowed until another terminal selection occurs.
...
- `latex_inline_edits`: accumulated inline editorial history containing each instruction, original selection, replacement, final Codex note, status, and rollback metadata.
- `latex_comments`: legacy client-local manuscript comments retained only as a one-time migration source when a project does not yet have `.panepilot/latex-comments.json`; new comment reads and writes use the shared project file.
...
- Text and generic binary previews are truncated at 1 MB. Supported image previews are capped at 5 MB.
...
- Explorer toolbar/context actions create empty files and folders or rename existing entries on local and SSH projects. Creation and rename accept a single basename, remain inside the canonical project root, reject collisions, and never overwrite. Monaco's context menu and the preview header can copy the full local or `ssh-alias:/path` reference, rename the saved file, and download it through the native Save dialog.
- Terminal links recognize project-contained path-like text and optional line/column suffixes. Unencoded trailing prose periods are excluded from the path and clickable range without changing internal extension dots or dotfiles. Clicking a link switches to Files, opens the authoritative file in Monaco, and reveals the requested line and column when present.
...
- The attachment is committed before tmux starts. New-session metadata therefore includes its LaTeX kind, scope, mode, and portable section identity in the first snapshot. Remote reconciliation repairs a stale generic snapshot from a known local attachment instead of demoting the chat.
...
- The PDF viewer keeps one continuous scroll surface, renders pages near the viewport, and stays mounted after first use so capability switching does not reset the reader. Its manual Recompile action invokes `latexmk` locally or over SSH with a three-minute/output bound and reloads the PDF without returning to page one.
- The renderer keeps the last successful workspace map per LaTeX project for its current lifetime. Reopening a known project renders that map immediately without an automatic source rescan; explicit Rescan and structural source saves refresh it in the background, and a failed background refresh does not replace the working editor.
- LaTeX source auto-save is on by default and stored per project on the current client. It debounces writes, keeps the manual Save action available, and refreshes the section map only when structural section/include commands change.
- Manuscript and PDF Preview stay mounted after first use and share the same mounted writing-chat margin. Their collapse control and accessible drag gutter update a project-scoped local layout preference, while switching between the two views preserves Monaco, PDF.js, and the active chat terminal.
- PDF Auto compile is off by default and persisted locally per project. While enabled for the active project, a bounded local/SSH source-revision check detects `.tex` changes and invokes the existing compile path; while disabled, no watcher or compiler runs, and only manual Recompile replaces the cached PDF snapshot.
- `.panepilot/latex-comments.json` is the authoritative, 1 MB-bounded project comment file for both local and SSH LaTeX projects. It rejects symlinks, is replaced atomically, and contains versioned comment records without a client-local project ID so the same folder can be opened by multiple PanePilot installations.
