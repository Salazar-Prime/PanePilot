...
48. Application readability is client-local and controls Electron's renderer zoom as one bounded scale, so interface text, xterm, Monaco, and rendered documents grow together without separate drifting preferences. Comfortable 110% is the first-run default; the toolbar exposes 90%, 100%, 110%, and 125% choices, while Command/Control + plus and zero remain keyboard controls. Command/Control + minus is reserved for workspace history.
