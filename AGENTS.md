...
64. Selecting saved or unsaved LaTeX source in the Manuscript Monaco editor exposes one anchored inline-edit prompt. PanePilot saves the draft before dispatch, verifies the exact saved file text and Monaco line/column range in the main process, and sends the request through a compatible persistent writing chat in Edit mode. A missing compatible chat opens a scope-locked Codex/Claude attachment flow and then sends the held request; inline editing never introduces a separate model backend or bypasses section scope.
