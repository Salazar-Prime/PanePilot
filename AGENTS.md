...
53. New Project supports either an existing folder or one newly created beneath a chosen local/SSH parent. The new folder accepts one validated name, never overwrites an existing entry, becomes the default editable project name, and is created before the project record. A new LaTeX folder also receives its normalized main file with a minimal document so the existing LaTeX validation remains true.
