...
47. A configured GitHub repository shows Public, Private, or Internal visibility in the top Repository action. PanePilot asks the locally authenticated GitHub CLI first so private repositories can be identified, falls back to GitHub's public repository API, caches the read briefly, and shows Unknown rather than guessing when neither source can see the repository. This indicator never changes repository visibility.
