...
56. A successfully loaded LaTeX workspace map is cached per project for the current renderer lifetime. Only the first uncached visit uses the blocking manuscript-mapping screen; later visits render the cached sections immediately and refresh them in the background. Late SSH responses must never replace the newly selected project, and a background refresh failure must leave the cached editor usable.
...
60. The LaTeX Document Map is independently hideable through a client-local, per-project layout preference. Source comments are durable project-scoped annotations attached to an exact saved `.tex` selection plus bounded surrounding anchors. Monaco highlights resolvable selections and renders visible-file comments in a proofing margin aligned to editor line positions; the cards follow editor scrolling, and a genuinely missing or ambiguous selection is marked as moved rather than silently reattached. Comment creation saves an unsaved draft first, validates the exact selection in the main process, and never invokes an agent.
...
- `latex_comments`: durable manuscript comments containing their project file, exact source selection, comment body, coordinates, and bounded text anchors.
...
- The renderer keeps the last successful workspace map per LaTeX project for its current lifetime. Reopening a known project renders that map immediately while a request refreshes it; only an uncached first visit blocks on manuscript mapping, and a failed background refresh does not replace the working editor.
...
- Manuscript and PDF Preview use the same mounted writing-chat margin. Its collapse control and accessible drag gutter update a project-scoped local layout preference, while switching between the two views preserves the active chat terminal.
