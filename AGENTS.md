...
68. Workspace navigation history is client-local and persists a bounded most-recently-used list spanning projects, ordinary terminal tabs, and type-specific capabilities. Command/Control + Up or Down opens a translucent switcher bounded to the left sidebar with at most seven valid prior destinations; the current destination is excluded, the first press selects the most recent destination, repeated Up/Down presses move the sliding highlight in either direction, and releasing the primary modifier opens the selected destination in the focused pane. While the modifier remains held, Left drills into the highlighted project's available capabilities and Right drills into all of its ordinary non-archived terminals; both child lists retain keyboard selection and scroll when necessary. The transient switcher is keyboard-only: pointer hover, selection, and clicks do not alter or open a destination. Each row leads with the project's current custom or fallback glyph and overlays a color-coded capability sub-icon. Only an ordinary terminal destination may show its exact terminal's Working, Needs attention, or Flagged indicator; Files, Notes, Actions, other capabilities, and LaTeX destinations never borrow aggregate project status. Archived or deleted projects and missing sessions are skipped. Escape or window blur cancels without navigating. Selecting a terminal expands its project when necessary and minimally scrolls that exact row into the visible area of the sidebar; manually collapsing the currently selected project remains allowed until another terminal selection occurs.
