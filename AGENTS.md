...
48. Application readability is client-local and controls Electron's renderer zoom as one bounded scale, so interface text, xterm, Monaco, and rendered documents grow together without separate drifting preferences. Comfortable 110% is the first-run default, and the toolbar exposes 90%, 100%, 110%, and 125% choices. Interface zoom has no keyboard shortcut: Command/Control + plus and zero are suppressed, while Command/Control + minus is reserved for workspace history.
...
68. Workspace navigation history is client-local and persists a bounded most-recently-used list spanning projects, ordinary terminal tabs, and type-specific capabilities. Command/Control + minus opens a translucent switcher bounded to the left sidebar with at most five valid prior destinations; the current destination is excluded, repeated taps move one sliding highlight, and releasing the primary modifier opens the selected destination in the focused pane. Each row leads with the project's current custom or fallback glyph and overlays a color-coded capability sub-icon. Only an ordinary terminal destination may show Working or Needs attention from that exact terminal's live lifecycle state; Files, Notes, Actions, other capabilities, and LaTeX destinations never borrow aggregate project status. Archived or deleted projects and missing sessions are skipped. Escape or window blur cancels without navigating.
