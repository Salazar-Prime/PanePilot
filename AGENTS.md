...
68. Workspace navigation history is client-local and persists a bounded most-recently-used list spanning projects, ordinary terminal tabs, and type-specific capabilities. Command/Control + minus opens a translucent switcher bounded to the left sidebar with at most five valid prior destinations; the current destination is excluded, repeated taps move one sliding highlight, and releasing the primary modifier opens the selected destination in the focused pane. Archived or deleted projects and missing sessions are skipped. Escape or window blur cancels without navigating.
