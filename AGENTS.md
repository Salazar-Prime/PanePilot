...
53. New Project supports either an existing folder or one newly created beneath a chosen local/SSH parent. The new folder accepts one validated name, never overwrites an existing entry, becomes the default editable project name, and is created before the project record. A new LaTeX folder also receives its normalized main file with a minimal document so the existing LaTeX validation remains true. In the SSH folder browser, typing after the current canonical path fuzzy-filters and ranks the visible child folders without making another remote request; selecting a result performs the navigation.
