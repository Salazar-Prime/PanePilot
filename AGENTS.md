...
37. Any Codex or Claude chat tab can be force reloaded in place, including one that never connected or linked a provider ID. PanePilot closes only that tab’s exact backend session and recreates tmux with the same name and unsafe-mode setting. It resumes the exact stored or newly discovered provider session ID when available; otherwise it starts a fresh provider chat in the same tab.
