...
64. Selecting saved or unsaved LaTeX source in the Manuscript Monaco editor exposes one anchored inline-edit prompt. PanePilot saves the draft before dispatch, verifies the exact saved file text and Monaco line/column range in the main process, and includes the selected paragraph plus one neighboring paragraph on each side as context. Each LaTeX project has at most one hidden, project-scoped, tmux-backed Codex inline-edit session; it is excluded from the sidebar and Writing chats pane, while a Save-adjacent Inline chat button reveals its latest interaction. Every new ask sends Codex's internal `/clear` command before the new prompt without replacing the tmux session. Edit snapshots and highlights are independent of that context reset and remain until the user explicitly clears them.
