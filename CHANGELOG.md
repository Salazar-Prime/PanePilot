...
- Automatic Codex/tmux reconnection after SSH loss or laptop wake, using shared
  `run-state` and `task-progress` snapshots instead of remote event spools.
- Codex, Claude Code, and login-shell terminal profiles with distinct icons.
- Editable project Actions that run commands in fresh ephemeral tmux sessions and retain only the latest output.
...
- In-memory Files workspace state that survives switching between project tabs.
